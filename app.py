...
# AWS Configuration - explicitly load credentials
AWS_REGION = os.getenv('AWS_REGION', 'us-east-1')
AWS_ACCESS_KEY_ID = os.getenv('AWS_ACCESS_KEY_ID')
AWS_SECRET_ACCESS_KEY = os.getenv('AWS_SECRET_ACCESS_KEY')

# AWS Polly client with explicit credentials
    ...
    with st.chat_message("assistant"):
        ...
        try:
            ...
            with st.spinner("🔍 Searching AWS Bedrock Knowledge Base..."):
                ...
                # Initialize Bedrock KB with explicit credentials
                kb = BedrockKnowledgeBase(
                    region_name=AWS_REGION,
                    aws_access_key_id=AWS_ACCESS_KEY_ID,
                    aws_secret_access_key=AWS_SECRET_ACCESS_KEY
                )
                response = kb.query_knowledge_base(modified_prompt)
